...
| `openstax-chemistry-2e` | `data/ions.toml` ion charges; `data/solubility.toml` rules; `data/elements.toml` electronegativity; `data/bonding.toml` ΔEN bond classes | `cited` | OpenStax *Chemistry 2e*, CC BY 4.0 (ion charges; solubility rules from Table 4.1, §4.2; Pauling electronegativities from Fig 7.6, §7.2; ΔEN bond classification from Fig 7.8, §7.2 — pure covalent < 0.4, polar covalent 0.4–1.8, ionic > 1.8, explicitly "a general guide with many exceptions") | common ion charges + solubility rules + Pauling electronegativities + the bond-class guideline (universal teaching facts; the Allred-revised Pauling scale); ion *composition* is separately machine-checked in `data.py`; the ΔEN caveat ships as data with the rule; accessed 2026-07-05, Fig 7.8 verified 2026-07-08 |
