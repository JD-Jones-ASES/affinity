...
trend machine-verified, the `comparison` lesson kind, ADR-0047; + 3 equilibrium — the ICE table = the species ledger with
the extent solved from mass action Q=K, the `equilibrium` lesson kind with three subtypes, ADR-0048: the weak-acid pH of acetic
acid + the weak-base pH of ammonia (water excluded from Q, pH via the Kw bridge — the solver reused unchanged) + the Ksp molar
solubility of CaF₂ — a pure solid excluded from Q, a cubic)
...
`data/bonding.toml` ΔEN rule, ADR-0031/0033) + 28 concept entries (incl. `chemical-equilibrium` + `ph` + `solubility-product` + `water-autoionization`, ADR-0048) + 7 reaction families (21 engine-classified
...
(dimensional homogeneity machine-checked, ADR-0039 — incl. Hess's law ΔH_rxn=Σν·ΔH_f°, ADR-0043) = **65 Atlas reference
objects**, 350 producer tests + 7 Node gates + `astro build` (33 pages) + live CI/Pages green. Lesson practice mass/leftover/volume/heat questions are
