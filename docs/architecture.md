...
**As-built (2026-07-08 — Phase 0 + Phase 1 complete + owner-reviewed; Phase 2 OPEN — the formula-sheet Atlas kind (ADR-0039), the gas-laws gym (ADR-0040), and the gas-stoichiometry lesson (ADR-0041) landed).** The pipeline runs end to end — compute → emit →
verify → **present**. The engine: `data/` (element, ion, solubility, acid-base, decomposition, constants datasets, ADR-0012/0017/0035/0039);
`chemkernel.data`/`formula`/`balance`/`units`/`dimension`/`extent`/`reaction`/`reactivity`/`solubility`/`build` (ADR-0012–0019/0035/0039);
...
molar masses and balancer in pytest. **Current counters: 5 lessons (2 precipitation + 1 percent-yield + 1
acid-base neutralization, ADR-0037; + 1 gas-stoichiometry — the ledger drives a gas volume via PV=nRT, ADR-0041)
+ 9 gyms (conversions + ionic nomenclature + balancing + mass
stoichiometry + percent yield + limiting reagent + periodic trends + reaction families + gas laws; 90 drills —
numeric families free-entry, ADR-0032), 1 Valence Table (23 elements; four modes — Explore lenses / Trends / Formula
builder / Bonding — over sourced properties + the 156-pair named crossover product + the `data/bonding.toml`
ΔEN rule, ADR-0031/0033) + 20 concept entries + 7 reaction families (21 engine-classified example reactions,
ADR-0035) + 14 species entries (composition/charge/molar mass derived from the formula + re-summed in Node,
ADR-0038) + 8 formula-sheet entries (dimensional homogeneity machine-checked, ADR-0039), 264 producer tests +
7 Node gates + `astro build` (23 pages) + live CI/Pages green. Lesson practice mass/leftover/volume questions are
free-entry too (ADR-0032); gas-stoichiometry practice re-derives from reaction constants with no interactive block (ADR-0041).**
...
| `units.py` engine | `Quantity` over an amount/mass/volume/**pressure/temperature** `Dim` basis (pressure + temperature added for gases, ADR-0040); exact Decimal; units cancel through ×/÷; rejects invalid conversions; certifies + computes the gas-law product (`(n·R·T/P).to("L")`). °C→K is an affine boundary conversion, never a scaling unit | **built** (ADR-0015, +gas dims ADR-0040) |
...
| `build.py` orchestration | authored `problems/**/*.problem.toml` → engine → verified `derived/<topic>/<slug>.solution.json`; entry point `build-problems`; exact decimal strings; three reported-product shapes — precipitate / water / **gas** (a weighed-`mass_g` given → moles, a `[conditions]` block → `result.gas` volume via PV=nRT through the units engine, ADR-0041) | **built** (ADR-0019, +gas ADR-0041) |
...
| `practice.py` generator | deterministic seeded variants off the reaction → solver-verified answers + misconception distractors; reject-list (near-ties, no leftover, colliding displays); reuses `interactive` multiplicities — or, for **gas stoichiometry** (`generate_gas_practice`, ADR-0041), re-derives from the reaction constants with no interactive (free-entry volume via PV=nRT + leftover; categorical limiting; the 22.4-L-at-STP + excess-reactant distractors) | **built** (ADR-0022/0041, two families) |
...
| `gym.py` drill generator | authored `gyms/**/*.gym.toml` → deterministic generated problem sets; exact Fractions (non-terminating rejected) for ledger-exact families, model-exact-then-rounded for gas laws (ADR-0040); dimensions re-proven through `units.py`; equations balanced by `balance.py`; named-mistake distractors; regime-2 families disclose a model assumption; `build-gyms` entry point | **built** (ADR-0024/0027/0028/0029/0034/0036/0040, **nine** families: conversions · ionic nomenclature · balancing · mass stoichiometry · percent yield · limiting reagent · periodic trends · reaction families · gas laws) |
...
| equilibrium / kinetics / thermo / electrochem | ICE-as-ledger, rate laws, energy ledger, electron ledger; the gas-law/calorimetry **formulas** are on the sheet (ADR-0039) — the numeric gas-law gym + a gas-stoichiometry lesson are the next Phase-2 increments | Phase 2 (open) |
...
gas's volume via PV=nRT for gas stoichiometry, ADR-0041 — plus `limiting_species`, `leftover`,
