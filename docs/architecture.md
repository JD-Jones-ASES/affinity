...
molar masses and balancer in pytest. **Current counters: 14 lessons (2 precipitation + 1 percent-yield + 1
...
trend machine-verified, the `comparison` lesson kind, ADR-0047; + 5 equilibrium — the ICE table = the species ledger with
...
solid excluded from Q, a cubic — plus a common-ion variant (CaF₂ into 0.10 M F⁻, dissolution suppressed 59 400×))
...
`data/bonding.toml` ΔEN rule, ADR-0031/0033) + 30 concept entries (incl. `chemical-equilibrium` + `ph` + `solubility-product` + `water-autoionization` + `buffer` + `common-ion-effect`, ADR-0048) + 7 reaction families (21 engine-classified
...
(dimensional homogeneity machine-checked, ADR-0039 — incl. Hess's law ΔH_rxn=Σν·ΔH_f°, ADR-0043) = **67 Atlas reference
objects**, 369 producer tests + 7 Node gates + `astro build` (36 pages) + live CI/Pages green. Lesson practice mass/leftover/volume/heat questions are
