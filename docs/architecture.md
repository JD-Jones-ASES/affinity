...
molar masses and balancer in pytest. **Current counters: 9 lessons (2 precipitation + 1 percent-yield + 1
...
trend machine-verified, the `comparison` lesson kind, ADR-0047)
...
`data/bonding.toml` ΔEN rule, ADR-0031/0033) + 24 concept entries + 7 reaction families (21 engine-classified
...
(dimensional homogeneity machine-checked, ADR-0039 — incl. Hess's law ΔH_rxn=Σν·ΔH_f°, ADR-0043) = **61 Atlas reference
objects**, 315 producer tests + 7 Node gates + `astro build` (30 pages) + live CI/Pages green. Lesson practice mass/leftover/volume/heat questions are
...
schema). It pins a **reaction** lesson; a single-molecule **structure** lesson is a different object (no equations /
species ledger / reported product) with its own tight `schemas/structure-lesson.schema.json` (ADR-0045), not a bent
union. The emitted object (see `derived/precipitation/calcium-carbonate-limiting.solution.json`) carries,
