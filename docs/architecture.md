...
**As-built (2026-07-08 — Phase 0 complete; Phase 1 complete pending owner review — items 1–6 + the Atlas breadth audit landed).** The pipeline runs end to end — compute → emit →
...
ADR-0038), 243 producer tests + 7 Node gates + `astro build` (20 pages) + live CI/Pages green. Lesson practice
...
| check-katex | **built** (ADR-0023/0035/0038): every LaTeX string renders through KaTeX with `throwOnError:true` — solution equations/ledger, concept/Valence-Table LaTeX, every reaction-family equation/net-ionic/species/general-form, and each species-atlas symbol + inline prose math |
