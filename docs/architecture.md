...
trend machine-verified, the `comparison` lesson kind, ADR-0047; + 4 equilibrium — the ICE table = the species ledger with
the extent solved from mass action Q=K, the `equilibrium` lesson kind with four subtypes, ADR-0048: the weak-acid pH of acetic
...
`data/bonding.toml` ΔEN rule, ADR-0031/0033) + 29 concept entries (incl. `chemical-equilibrium` + `ph` + `solubility-product` + `water-autoionization` + `buffer`, ADR-0048) + 7 reaction families (21 engine-classified
...
(dimensional homogeneity machine-checked, ADR-0039 — incl. Hess's law ΔH_rxn=Σν·ΔH_f°, ADR-0043) = **66 Atlas reference
objects**, 356 producer tests + 7 Node gates + `astro build` (34 pages) + live CI/Pages green. Lesson practice mass/leftover/volume/heat questions are
