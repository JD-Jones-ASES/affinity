...
molar masses and balancer in pytest. **Current counters: 15 lessons (2 precipitation + 1 percent-yield + 1
...
trend machine-verified, the `comparison` lesson kind, ADR-0047; + 6 equilibrium — the ICE table = the species ledger with
the extent solved from mass action Q=K, the `equilibrium` lesson kind with five subtypes, ADR-0048: the weak-acid pH of acetic
...
polyprotic staged pH of H₃PO₄ (Kₐ1≫Kₐ2≫Kₐ3, the solver run once per stage on the previous stage's output))
...
`data/bonding.toml` ΔEN rule, ADR-0031/0033) + 31 concept entries (incl. `chemical-equilibrium` + `ph` + `solubility-product` + `water-autoionization` + `buffer` + `common-ion-effect` + `polyprotic-acid`, ADR-0048) + 7 reaction families (21 engine-classified
...
(dimensional homogeneity machine-checked, ADR-0039 — incl. Hess's law ΔH_rxn=Σν·ΔH_f°, ADR-0043) = **68 Atlas reference
objects**, 376 producer tests + 7 Node gates + `astro build` (37 pages) + live CI/Pages green. Lesson practice mass/leftover/volume/heat questions are
