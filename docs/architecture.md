...
`data/bonding.toml` ΔEN rule, ADR-0031/0033) + 33 concept entries (incl. `chemical-equilibrium` + `ph` + `solubility-product` + `water-autoionization` + `buffer` + `common-ion-effect` + `polyprotic-acid` + `titration` + `reaction-quotient`, ADR-0048) + 7 reaction families (21 engine-classified
...
as **dimensionless activity** relations, ADR-0048) = **75 Atlas reference
objects**, 394 producer tests + 7 Node gates + `astro build` (40 pages) + live CI/Pages green. Lesson practice mass/leftover/volume/heat questions are
...
and **prediction** (ADR-0048 9th increment — the Q-vs-Kₛₚ snapshot: a `mixing` block + a `quotient` + a verdict, no ICE table)
...
| check-katex | **built** (ADR-0023/0035/0038/0039/0044/0045): every LaTeX string renders through KaTeX with `throwOnError:true` — solution equations/ledger, concept/Valence-Table LaTeX, every reaction-family equation/net-ionic/species/general-form, each species-atlas symbol + inline prose math, each formula-sheet statement + rearrangement + inline math, each molecule's formula symbol + inline summary/polarity/notes math, each **structure lesson**'s molecule symbol + inline scenario/step/misconception/polarity/assumption math (ADR-0045), each **equilibrium lesson**'s reaction (⇌) + mass-action expression + ICE species symbols + inline scenario/assumption/misconception math (ADR-0048), and each **prediction lesson**'s dissolution reaction + Kₛₚ/Q expressions + source/ion symbols + inline scenario/assumption/misconception math (ADR-0048 9th increment) |
