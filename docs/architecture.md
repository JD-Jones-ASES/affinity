...
molar masses and balancer in pytest. **Current counters: 10 lessons (2 precipitation + 1 percent-yield + 1
...
trend machine-verified, the `comparison` lesson kind, ADR-0047; + 2 equilibrium — the ICE table = the species ledger with
the extent solved from mass action Q=K, the `equilibrium` lesson kind with two subtypes, ADR-0048: the weak-acid pH of acetic
acid + the Ksp molar solubility of CaF₂ — a pure solid excluded from Q, a cubic)
...
`data/bonding.toml` ΔEN rule, ADR-0031/0033) + 27 concept entries (incl. `chemical-equilibrium` + `ph` + `solubility-product`, ADR-0048) + 7 reaction families (21 engine-classified
...
(dimensional homogeneity machine-checked, ADR-0039 — incl. Hess's law ΔH_rxn=Σν·ΔH_f°, ADR-0043) = **64 Atlas reference
objects**, 340 producer tests + 7 Node gates + `astro build` (32 pages) + live CI/Pages green. Lesson practice mass/leftover/volume/heat questions are
...
| `data.py` data layer | loads `data/` datasets (elements + **periodic properties**, ions, solubility, **constants**, **specific heats**, **formation enthalpies**, **VSEPR geometry**, **boiling points**); molar mass; the Avogadro + gas constants; ΔH_f° by (formula, phase); the VSEPR table keyed (domains, lone pairs); boiling points by formula (IMF evidence, ADR-0046); acid ionization constants K_a by acid formula + solubility products K_sp by salt formula (with ion counts derived by charge crossover + the salt composition machine-checked, ADR-0048); the only path to empirical values (ADR-0006); self-validates on load | **built** (ADR-0012, +constants ADR-0030, +properties ADR-0031, +specific heats ADR-0042, +ΔH_f° ADR-0043, +VSEPR ADR-0044, +boiling points ADR-0046, +K_a/K_sp ADR-0048) |
...
| `equilibrium.py` reversible-extent solver | the ICE table = the species ledger with the extent solved from **mass action** (Q(x)=K) not the limiting reagent — `solve_equilibrium` finds the root by **bisection to high precision** (exact Decimal, general beyond the quadratic), the extent model-exact-then-rounded (ADR-0040 pattern), the machine-check the residual Q(committed)=K; an `in_quotient` flag excludes a **pure solid** from Q (grows the bracket when no quotient reactant limits — the Ksp case). Two builders: `build_equilibrium_lesson` (weak-acid pH — dissociation DRY-sourced from acids-bases, K_a from ionization-constants, pH=−log₁₀[H⁺]) + `build_solubility_lesson` (Ksp — the solid excluded, K_sp=4s³ a **cubic**, → molar solubility s + g/L). Refuses an unbracketed root / a strong or polyprotic acid / an unknown salt | **built** (ADR-0048) |
