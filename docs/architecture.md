...
trend machine-verified, the `comparison` lesson kind, ADR-0047; + 1 equilibrium — the ICE table = the species ledger with
the extent solved from mass action Q=K, the `equilibrium` lesson kind, ADR-0048: the weak-acid pH of acetic acid)
...
`data/bonding.toml` ΔEN rule, ADR-0031/0033) + 26 concept entries (incl. `chemical-equilibrium` + `ph`, ADR-0048) + 7 reaction families (21 engine-classified
...
(dimensional homogeneity machine-checked, ADR-0039 — incl. Hess's law ΔH_rxn=Σν·ΔH_f°, ADR-0043) = **63 Atlas reference
objects**, 333 producer tests + 7 Node gates + `astro build` (31 pages) + live CI/Pages green. Lesson practice mass/leftover/volume/heat questions are
