...
molar masses and balancer in pytest. **Current counters: 19 lessons (2 precipitation + 1 percent-yield + 1
...
other verdict; + 1 kinetics — the ledger in TIME: a first-order reactant decays [A]=[A]₀·e^(−kt), half-life t½=ln2/k
(concentration-independent), the `kinetics` lesson kind with a build-time decay curve, ADR-0049: H₂O₂ decomposition, t½=6.00 h)
...
`data/bonding.toml` ΔEN rule, ADR-0031/0033) + 36 concept entries (incl. `chemical-equilibrium` + `ph` + `solubility-product` + `water-autoionization` + `buffer` + `common-ion-effect` + `polyprotic-acid` + `titration` + `reaction-quotient`, ADR-0048; + `reaction-rate` + `rate-law` + `half-life`, ADR-0049) + 7 reaction families (21 engine-classified
...
as **dimensionless activity** relations, ADR-0048) = **78 Atlas reference
objects**, 404 producer tests + 7 Node gates + `astro build` (41 pages) + live CI/Pages green. Lesson practice mass/leftover/volume/heat questions are
...
| `kinetics.py` first-order decay | the species ledger with the extent evolving in **time** (ADR-0049): `concentration_first_order` ([A]=[A]₀·e^(−kt), Decimal `.exp()`), `half_life_first_order` (t½=ln2/k, Decimal `.ln()`) — the transcendental decay is **model-exact-then-rounded** (ADR-0040 pattern). `build_kinetics_lesson` requires **first order** (k + order from `data/rate-constants.toml`, sourced), samples the decay curve at multiples of t½, emits the halving landmarks, and machine-checks the balance (a small element-conservation parser) + t½. Refuses a non-first-order reaction / unknown reactant / nonpositive input | **built** (ADR-0049) |
...
and **kinetics** (ADR-0049 — the ledger in time: a rate law + integrated law + half-life + a decay curve, no completion ledger)
