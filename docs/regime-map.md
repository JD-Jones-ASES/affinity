...
| Lewis structures | 1, 4 | 2 | **atlas + gym** (molecule kind + `lewis_structures_v1`, ADR-0044): the **Lewis electron ledger** — valence total, octet/duet, formal charge, and their conservation — **machine-checked** (exact integer accounting, re-derived in pure Node); the gym drills valence-total + electron-domain counting (free-entry). Octet exceptions + resonance deferred |
| VSEPR & polarity | 3, 4 | 2 | **atlas + gym** (molecule kind + `lewis_structures_v1`, ADR-0044): VSEPR geometry from the sourced `data/vsepr.toml` table keyed on the machine-derived electron-domain count; the gym drills molecular-shape (categorical, electron-domain-geometry distractor). Molecular polarity authored + disclosed (model-assumed, neutral molecules) |
