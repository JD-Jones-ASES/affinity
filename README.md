...
**Status:** **Phase 2 open** (Phase 1 complete + owner-reviewed) — twelve machine-verified lessons (precipitation,
...
boiling-point trend is itself machine-checked (CH₄ ≪ NH₃ ≪ H₂O)), and three **equilibrium** lessons (the pH of
acetic acid, the pH of ammonia — a weak base, the exact mirror of the acetic-acid lesson across neutral, water excluded from
Q and pH reached through the Kw bridge — and the Ksp molar solubility of calcium fluoride — the ICE table is the species
ledger with the extent solved from **mass action** Q=K, not driven to a limiting reagent; for Ksp the pure solid is excluded
from Q, so it is a cubic, and the machine-check is the residual), eleven procedural gyms (dimensional analysis, nomenclature,
...
reversible-extent solver + the `equilibrium` lesson kind with three subtypes — weak-acid pH, weak-base pH, Ksp solubility);
